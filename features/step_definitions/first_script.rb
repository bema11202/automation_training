...
describe 'Puppy adoption' do
  ...
  wait.until { browser.find_element(:class, 'rounded_button').click }

  wait.until { browser.find_element(:class, 'rounded_button').click }
  wait.until { browser.find_element(:type, 'submit').click }


  name = browser.find_element(:id, 'order_name')
  ...
  # selection = option.options.sample
  # selection.click
